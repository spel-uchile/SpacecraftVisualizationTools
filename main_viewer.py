...
from forms.desktop.main_screen_visual_simulator_ui import Ui_MainWindow
import matplotlib.pyplot as plt
from matplotlib.backends.backend_qt5agg import FigureCanvas
from matplotlib.figure import Figure
from datetime import datetime
from forms.initial_date_ui import InitDateForm
from Visualization.SpacecraftVisualizationTools.forms.spacecraft.untitled_ui import Ui_Form
from Visualization.SpacecraftVisualizationTools.forms.desktop.pruebaDock_ui import Ui_Form
...
class Viewer(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        ###
        # Main window buttons/actions connections
        ###
        self.window.actionAddSatellite.triggered.connect(self.add_new_satellite)
        ...
        tabs.removeTab(7)
        tabs.removeTab(7)
        self.window.actionAttSetting.triggered.connect(self.open_tab_attitude_sett)
        self.window.actionOrbSetting.triggered.connect(self.open_tab_orbit_sett)
        self.window.actionSCSetting.triggered.connect(self.open_tab_sc_sett)

        # TODO: quitar variables
        self.time_speed = 1
    ...
    def add_new_satellite(self):
        # CREATE TAB SPACECRAFT
        #self.get_spacecraft_dataset()

        # ADD to TREE list
        _translate = QtCore.QCoreApplication.translate
        item_1 = QtWidgets.QTreeWidgetItem(self.window.treeWidget.topLevelItem(0), ["SUCHAI-2"])
        item_1.setCheckState(0, QtCore.Qt.Checked)
        item_2 = QtWidgets.QTreeWidgetItem(item_1, ["Gyrospcope"])
        item_2.setCheckState(0, QtCore.Qt.Checked)
        return

    def open_tab_sc_sett(self):
        self.window.tabWidget.addTab(self.window.tab_sc_properties, "Spacecraft properties")
        self.window.tabWidget.setCurrentIndex(len(self.window.tabWidget) - 1)
        #self.window.tab_spacecraft.layout = QMainWindow.addDockWidget()
        #self.window.tab_spacecraft.addWidget(QLabel("ffff"))
        return

    def open_tab_orbit_sett(self):
        self.window.tabWidget.addTab(self.window.tab_orbit_properties, "Orbit properties")
        self.window.tabWidget.setCurrentIndex(len(self.window.tabWidget) - 1)

    def open_tab_attitude_sett(self):
        self.window.tabWidget.addTab(self.window.tab_attitude_properties, "Attitude properties")
        self.window.tabWidget.setCurrentIndex(len(self.window.tabWidget) - 1)
